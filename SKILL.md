...
**探活异常容错**：若 `probe_entry` 抛出非「不存在」类异常（如代理模式网络超时），跳过该文档并保留 manifest 映射（不 remove_entry、不中断整个同步），下次重跑时自动重试。避免因探活异常冲掉映射导致后续重复创建。

### 2.1 创建前查重（防重复创建）⚠️ 核心防护

**问题背景**：manifest 可能因断连/代理异常等丢失映射（如 07-08 连接器断开 → probe 失效 → remove_entry → 重建失败 → 映射被冲掉）。映射丢失后，下一次同步会把已有文档当「新文档」处理，在乐享侧创建重复副本。

**防护机制**：当判定为「新增」（manifest 无记录或类型不匹配）时，**先调 `list_children` 查乐享目标父目录是否已有同名 entry**：
- **命中同名 page/file** → 复用其 entry_id，写入 manifest，走更新逻辑（用当前 Obsidian 内容覆盖乐享侧已有文档）→ **不创建新文档**
- **未命中** → 正常创建（真正的首次同步或新文档）

查重结果按 parent_id 缓存（同一目录只 list 一次），性能开销极小。dry-run 模式不查重（无网络调用）。

> 此机制确保：**即使 manifest 完全丢失，也不会在乐享侧创建重复文档**——最坏情况是多一次 list_children + 更新操作，而非多一份重复副本。
